...
from mjrl.algos.cpo import CPO
...
assert any([job_data['algorithm'] == a for a in ['NPG', 'BCRL', 'DAPG', 'TRPO', 'CPO']])
...
cost_baseline = MLPBaseline(e.spec, reg_coef=1e-3, batch_size=job_data['vf_batch_size'],
                       epochs=job_data['vf_epochs'], learn_rate=job_data['vf_learn_rate'])
...
if job_data['algorithm'] == 'TRPO':
    rl_agent = TRPO(e, policy, baseline,
                normalized_step_size=job_data['rl_step_size'],
                seed=job_data['seed'], save_logs=True
                )
elif job_data['algorithm'] == 'CPO':
    rl_agent = CPO(e, policy, baseline, cost_baseline,
                normalized_step_size=job_data['rl_step_size'],
                seed=job_data['seed'], save_logs=True,
                cost_lim=job_data['cpo_cost_lim']
                )
else:
