...
class MLP:
    ...
    def get_fim(self, x):
        ...
        cov_inv = self.old_log_std.exp().pow(-2).squeeze(0).repeat(x.size(0)) 
        param_count = 0
        std_index = 0
        id = 0
    ...
    def get_kl(self, x):
        # mean1, log_std1, std1 = self.get_action(x)
        action = self.get_action(x)
        mean1 = action[1]['mean']
        log_std1 = action[1]['log_std']
        std1 = np.exp(action[1]['log_std'])
