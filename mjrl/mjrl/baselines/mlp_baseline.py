...
class MLPBaseline:
    ...
    def _features(self, paths, type='rewards'):
    ...
    def fit(self, paths, type='rewards', return_errors=False):

        key = 'returns'
        if type is 'costs':
            key = 'cost_returns'

        featmat = self._features(paths, type)
        returns = np.concatenate([path[key] for path in paths]).reshape(-1, 1)
    ...
    def predict(self, path, type='rewards'): # other type can be `costs`
        featmat = self._features([path], type).astype('float32')
        if self.use_gpu:
