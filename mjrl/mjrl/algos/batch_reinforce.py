...
class BatchREINFORCE:
    ...
    def train_step(self, N,
        ...
        if self.cost_baseline:
            # compute cost returns
            process_samples.compute_cost_returns(paths, gamma)
            # compute cost advantages
            process_samples.compute_cost_advantages(paths, self.cost_baseline, gamma, gae_lambda)
        # train from paths
        ...
        if self.save_logs:
            ...
            if self.cost_baseline:
                ts = timer.time()
                cost_error_before, cost_error_after = self.cost_baseline.fit(paths, type='costs', return_errors=True)
                self.logger.log_kv('time_cost_VF', timer.time()-ts)
                self.logger.log_kv('VF_cost_error_before', cost_error_before)
                self.logger.log_kv('VF_cost_error_after', cost_error_after)
        else:
            self.baseline.fit(paths)
            if self.cost_baseline:
                self.cost_baseline.fit(paths, type='costs')
    ...
    def log_rollout_statistics(self, paths):
        ...
        min_cost = np.amin(path_costs)
        max_cost = np.amax(path_costs)
        self.logger.log_kv('cost_mean', mean_cost)
        self.logger.log_kv('cost_std', std_cost)
        self.logger.log_kv('cost_max', max_cost)
        self.logger.log_kv('cost_min', min_cost)
        try:
            success_rate, _ = self.env.env.env.evaluate_success(paths)
