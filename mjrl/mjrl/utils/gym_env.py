...
class GymEnv(object):
    ...
    def visualize_policy(self, policy, horizon=1000, num_episodes=1, mode='exploration', **kwargs):
        ...
        for ep in range(num_episodes):
            print("Episode %d" % ep)
            o = self.reset()
            if "init_dict" in kwargs:
                print("Set init dict")
                self.set_env_state(kwargs["init_dict"])
            d = False
            t = 0
            score = 0.0
            cost = 0.0
            ...
            while t < horizon and d is False:
                ...
                cost = cost + goal_achieved['constraint_violation']
                score = score + r
                if (kwargs['render']):
                    self.render()
                t = t+1
            ...
            total_score += score
            total_cost += cost
            print("Episode score = %f, Success = %d" % (score, sum(episode_data['goal_achieved']) > success_threshold), "Total Cost = ", total_cost)
        print("Average score = %f" % (total_score / num_episodes))
        print("Average cost = %f" % (total_cost / num_episodes / 0.01))
