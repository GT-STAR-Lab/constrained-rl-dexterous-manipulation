...
def compute_cost_returns(paths, gamma):
    for path in paths:
        path["cost_returns"] = discount_sum(path["costs"], gamma)
...
def compute_cost_advantages(paths, baseline, gamma, gae_lambda=None, normalize=False):
    # compute and store returns, advantages, and baseline 
    # standard mode
    if gae_lambda == None or gae_lambda < 0.0 or gae_lambda > 1.0:
        for path in paths:
            path["cost_baseline"] = baseline.predict(path, type='costs')
            path["cost_advantages"] = path["cost_returns"] - path["cost_baseline"]
        if normalize:
            alladv = np.concatenate([path["cost_advantages"] for path in paths])
            mean_adv = alladv.mean()
            std_adv = alladv.std()
            for path in paths:
                path["cost_advantages"] = (path["cost_advantages"]-mean_adv)/(std_adv+1e-8)
    # GAE mode
    else:
        for path in paths:
            b = path["cost_baseline"] = baseline.predict(path, type='costs')
            if b.ndim == 1:
                b1 = np.append(path["cost_baseline"], 0.0 if path["terminated"] else b[-1])
            else:
                b1 = np.vstack((b, np.zeros(b.shape[1]) if path["terminated"] else b[-1]))
            td_deltas = path["costs"] + gamma*b1[1:] - b1[:-1]
            path["cost_advantages"] = discount_sum(td_deltas, gamma*gae_lambda)
        if normalize:
            alladv = np.concatenate([path["cost_advantages"] for path in paths])
            mean_adv = alladv.mean()
            std_adv = alladv.std()
